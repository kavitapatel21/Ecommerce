...
class Mentionify {
  ...
  onInput(ev) {
	...
	var ref_id = jQuery(this.ref).closest('.cvt-accordion-body-content.open').attr('id');
	var token_height = jQuery("#"+ref_id+" .smsalert_tokens").height();
...
jQuery('.token-area').on("keypress", function(e) {
	if (e.keyCode == 64) {
		var textarea_id = e.target.id;
		var menuarea_id = jQuery(this).closest('div').find('.sa-menu-token').attr('id');
...
function createJSON(e) {
		...
		//for custom sms
		var custom_data = jQuery('#custom_token_list').html();
		if(custom_data != '' && typeof custom_data != 'undefined'){
			var parse_custom_data =  JSON.parse(custom_data);	
			jsonObj = parse_custom_data;
		} else {
			jQuery(".cvt-accordion-body-content.open .smsalert_tokens a, .cvt-accordion-body-content.active .smsalert_tokens a, fieldset .smsalert_tokens a, .otp-section-token .smsalert_tokens a").each(function() {
				if(jQuery(this).html() != '[...More]'){
					var key = jQuery(this).attr("data-val");
		...
		var jsonObj = jQuery.map(jsonObj, function(value, i) {return {key:value.key, val:value.val}});
	...
	if(id != ''){
		jQuery.ajax({
			...
			success: function(data)
			{
				...
				jQuery('#custom_token_list').html(JSON.stringify(content1) );
			},
			error:function (e,o){
			}
		});
...
{	
	...
	if(jsonObj2.length > 0){
		var finalobj 	= jQuery.map(jsonObj2, function(value, i) {return {key:value.key, val:value.val}});
		var users1 		= finalobj;
		return users1;
	}
}
...
jQuery("body").on('click', function(){
	jQuery('.sa-menu-token').attr('hidden','hidden');
});
/**@ search ends*/
...
class WooCommerceCheckOutForm extends FormInterface {
	...
	public function woocommerce_site_checkout_errors( $data, $errors ) {
		...
		global $phoneLogic;
		if ( ! isset( $user_phone ) || ! SmsAlertUtility::validatePhoneNumber( $user_phone ) ) {
		...
		if(isset($_POST['createaccount']) && $_POST['createaccount'])
		{
			...
			{
			$errors->add( 'registration-error-email-exists', 'An account is already registered with your email address. Please login.' );
			}
			if(isset($_POST['account_username']))
			{
				if(username_exists($_POST['account_username']))
				{
					$errors->add( 'registration-error-username-exists', 'An account is already registered with that username. Please choose another.' );
				}
			}
			...
			if ( count( $getusers ) > 0 ) {
				$errors->add( 'registration-error-number-exists', 'An account is already registered with this mobile number. Please login.' );
			}
		}
		}
    ...
    jQuery("input[name=payment_method],input[name=radio-control-wc-payment-method-options]").click(function() {
